...
interface SheetBookItem {
  ...
  is_active: boolean;
  created_at: string;
  /** 악보집 장르(서브카테고리) 이름. junction에서 파생 */
  book_genre?: string;
}
...
interface BookFormData {
  ...
  book_genre: string;      // 악보집 장르(서브카테고리). '' = 선택 안 함
  pdf_file: File | null;
}
...
// ─── Component ───────────────────────────────────────
export default function SheetBookManagement() {
  const [sheetBookCategoryId, setSheetBookCategoryId] = useState<string | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // 1. 악보집 메인 카테고리 찾기
      const { data: sheetBookCat } = await supabase
        .from('categories')
        .select('id')
        .eq('name', SHEET_BOOK_MAIN_CATEGORY_NAME)
        .maybeSingle();

      let catId = sheetBookCat?.id as string | undefined;
      if (!catId) {
        const { data: created } = await supabase
          .from('categories')
          .insert({ name: SHEET_BOOK_MAIN_CATEGORY_NAME, description: '악보집(PDF 통합본) 카테고리' })
          .select('id')
          .single();
        catId = created?.id;
      }
      if (catId) setSheetBookCategoryId(catId);
      ...
      // 1-b. 악보집 장르(서브카테고리) 카테고리 ID 맵 로드
      const { data: typeCats } = await supabase
        .from('categories')
        .select('id, name')
        .in('name', SHEET_BOOK_GENRE_OPTIONS);
      ...
      if (bookIds.length > 0) {
        ...
        (jrows || []).forEach((r: any) => {
          ...
          if (nm && SHEET_BOOK_GENRE_OPTIONS.includes(nm)) typeBySheet[r.sheet_id] = nm;
        });
      }
      const withType = (list || []).map((b: any) => ({ ...b, book_genre: typeBySheet[b.id] || '' }));
      setBooks(withType as SheetBookItem[]);
    } catch (error) {
      console.error('악보집 데이터 로드 오류:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handlePdfUpload = async (
    ...
    try {
      ...
      const safeName =
        sanitized.length > 2
          ? sanitized
          : `sheet_book_${Math.random().toString(36).substring(2, 10)}.${fileExt}`;
  ...
  const handleLessonDetailImageUpload = async (file: File): Promise<string> => {
    ...
    const fileName = `sheet_book_detail_${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExt}`;
    const filePath = `sheet-book-detail-images/${fileName}`;
    const { error: uploadError } = await supabase.storage
      .from('drum-sheets')
  ...
  const handleAddBook = async () => {
    ...
    if (!sheetBookCategoryId) {
      alert('악보집 카테고리가 없습니다. 페이지를 새로고침해주세요.');
      return;
    }
    if (form.price < 0) {
    ...
    try {
      ...
      const insertData: any = {
        ...
        category_id: sheetBookCategoryId,
      ...
      let baseSlug = slugRaw
        ...
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '');
      if (!baseSlug) baseSlug = `sheet-book-${Date.now()}`;
      ...
      if (form.book_genre && inserted?.id) {
        const typeCatId = await ensureTypeCategoryId(form.book_genre);
        if (typeCatId) {
          await supabase
            .from('drum_sheet_categories')
  ...
  // ── Edit Book ──
  const handleOpenEdit = (book: SheetBookItem) => {
    setEditingBook(book);
  ...
  const handleUpdateBook = async () => {
    ...
    try {
      ...
      if (editForm.book_genre) {
        const typeCatId = await ensureTypeCategoryId(editForm.book_genre);
        if (typeCatId) {
          await supabase
            .from('drum_sheet_categories')
  ...
  const handleDeleteBook = async (bookId: string) => {
    if (!confirm('이 악보집을 삭제하시겠습니까?')) return;
    try {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">📚 악보집 관리</h2>
          <p className="text-sm text-gray-500 mt-1">
            PDF 통합 악보집을 등록·관리합니다. (가격, 상세 페이지·목차 작성 가능)
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <span className="text-xs font-normal text-gray-500">
                  (악보집 코너 장르 필터에 사용 · 선택 사항)
                </span>
              </label>
              <select
                value={form.book_genre}
                onChange={(e) => setForm({ ...form, book_genre: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
              >
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <input
                    ...
                    className="w-28 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm"
                  />
                ...
                {form.pdf_url && !isUploadingPdf && (
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <button
                      ...
                      className="text-sm px-3 py-1.5 rounded-lg bg-white border border-indigo-300 text-indigo-800 hover:bg-indigo-50"
                    >
            ...
            <div>
              ...
              <input
                ...
                placeholder="예: 120"
                min="0"
                className="w-full sm:w-48 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-gray-100 flex justify-end gap-3 flex-shrink-0">
          ...
          <button
            ...
            className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-60 flex items-center gap-2"
          >
            {isSubmitting ? (
              <>
